...
double newtonRaphson(double* poly, int polyOrder, double p0, double TOL, int N, int plot);
...
int main(int argc, char *argv[])
{
	...
	plot = (int) strtoul(argv[6], 0, 10);

	poly = stringToDoublePoly(argv[1], order);
	
	zero = newtonRaphson(poly, order, p0, tol, n, plot);
	printf("\n\nf(x) = 0; x = %f\n", zero);
...
double newtonRaphson(double* poly, int polyOrder, double p0, double TOL, int N, int plot)
{
	double* derivative = polyDerivative(poly, polyOrder);
	int i = 0;
	double p = 0;
	double f = 0;
	double fLinha = 0;
	...
	while (i < N)
	{
		...
		f =  polyValue(poly, polyOrder, p0);
		fLinha =  polyValue(derivative, polyOrder, p0);
		p = p0 - (f/fLinha);
