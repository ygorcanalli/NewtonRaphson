...
double secante(double* poly, int polyOrder, double p0,  double p1, double TOL, int N, int plot);
...
int main(int argc, char *argv[])
{
	...
	zero = secante(poly, order, p0, p1, tol, n, plot);
	printf("\n\nf(x) = 0; x = %f\n", zero);
...
double secante(double* poly, int polyOrder, double p0, double p1, double TOL, int N, int plot)
{
	...
	while (i < N)
	{
		...
		q0 = q1;
		p1 = p;
		q1 = polyValue(poly, polyOrder, p); //q1 = f(p)
